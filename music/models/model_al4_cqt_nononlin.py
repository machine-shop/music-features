...
## constant q transform feature; only 1 session music
## 29 band tonotopy range, zscoring stimuli and responses

## NOTE: can't run this script w/o modification first b/c i reorganized
## the cloud directories. would need to adjust to be extracting stimuli from
## bucket 'loganesian_stimuli' and directory 'music_cqt/full_tonotopy_range'

## trim the first and last 5 TRs; no nonlinearity applied before temporal downsampling
## single alpha, 10 bootstraps, fit only auditory cortex
## using uncalibrated soundfiles initially
...
for i in range(len(stimuli)):
    ...
    CQT = inface.download_raw_array(stim_storage.format(stimuli[i]))
...
modelnames = ['cqtband{0}'.format(i) for i in range(stim_mats[stimuli[i]].shape[1])]
...
inface.dict2cloud(outfile, dict(models=['cqt'],
                              wt=wt,
                              corr=corr,
