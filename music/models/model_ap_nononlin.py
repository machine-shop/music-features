...
## This is the extracted pitches that we're working with
## pilot analysis to see if it would be worth while to pursue pitch

## one session of data
## used a single alpha, 10 bootstraps, no nonlinearity applied to the features
## before it was used in the regression
## fit only on auditory cortex, 4 delays, zscored stimuli and responses,
## trim the first and last five TRs
...
stim_storage = 'music_pitches/{0}.npy'
...
for i in range(len(stimuli)):
    # load the extracted pitches for the stimulus
    pitches = inface.download_npy_array(stim_storage.format(stimuli[i]))
    ...
    # calculate the old times of the pitch data
    oldtimes = np.arange(len(pitches)) * binsize

    # downsample cqt mat
    pitchesdownsamp = interpdata.lanczosinterp2D(pitches[:,None],
                                             oldtimes,
                                             newtimes, window=3,
                                             cutoff_mult=1.0,
                                             rectify=False)
    ...
    stim_mats[stimuli[i]] = pitchesdownsamp
